my <- function (formula, data, weights, subset, na.action, model = FALSE, 
    ...
    ...) 
{
    ...
    mf <- match.call(expand.dots = FALSE)#returns a call in which all of the specified arguments are specified by their full names.
    ...
    mf[[1L]] <- quote(stats::model.frame)#the mf is a model.frame, but it looks like a data.frame except it has more attributes.
    mf <- eval(mf, parent.frame())#the mf has been evaluated to be the model.frame.
    if (match.arg(method) == "model.frame") 
        return(mf)
    mt <- attr(mf, "terms")#terms attribute in model.frame.
    y <- model.response(mf, "numeric")
    w <- model.weights(mf)
    if (is.null(w)) 
        w <- rep(1, length(y))
    nmx <- as.character(attr(mt, "variables"))[-(1L:2)]#nmx is the independent variables name.
    x <- mf[, nmx, drop = FALSE]
    if (any(sapply(x, is.factor))) 
        stop("predictors must all be numeric")
    x <- as.matrix(x)
    D <- ncol(x) #D is the number of dimention of independent variables.
    nmx <- setNames(nm = colnames(x)) #interesting function of R, setNames. Much better than what I usually do.
    drop.square <- match(nmx, nmx[drop.square], 0L) > 0L #not sure if this is nessary, since drop.square is still giving us the logical vector which tells us which independent variables will be dropped for qudratic term.
    parametric <- match(nmx, nmx[parametric], 0L) > 0L #same as above, paramtric, should any terms be fitted globally rather than locally
    if (!match(degree, 0L:2L, 0L)) 
        stop("'degree' must be 0, 1 or 2")
    iterations <- if (family == "gaussian") 
        1
    else control$iterations #the default iteration time from control is 4.
    ...
    fit <- mysimple(y, x, w, span, degree, parametric, drop.square, 
...
mysimple <- function (y, x, weights, span = 0.75, degree = 2L, parametric = FALSE, 
    ...
    surface = "interpolate", cell = 0.2, iterations = 1L, trace.hat = "exact") 
{
    ...
    max.kd <- max(N, 200)
    robust <- rep(1, N)
    divisor <- rep(1, D) #initialize trimmed standard deviation
    if (normalize && D > 1L) {
        trim <- ceiling(0.1 * N) #how many observations will be dropped becaused of 10% trimmed standard deviation to one.
        divisor <- sqrt(apply(apply(x, 2L, sort)[seq(trim + 1, 
            N - trim), , drop = FALSE], 2L, var)) #divisor is the 10% trimmed standard deviation.
        x <- x/rep(divisor, rep(N, D)) # x now tend to be that 10% trimmed standard deviation to one.
    }
