		...
		int *sum_drop_sqr, int *setLf)
{
    ...
    nvmax = max(200, N);
    nf = min(N, (int) floor(N * (*span) + 1e-5)); /*nf is the number of observations that will be included in the local regression*/
    if(nf <= 0) error(_("span is too small"));
    tau0 = ((*degree) > 1) ? (int)((D + 2) * (D + 1) * 0.5) : (D + 1); /*condition ? value_if_true : value_if_false*/
    tau = tau0 - (*sum_drop_sqr); /*sum_drop_sqr is the number of independent variables that dropping the square term*/
    ...
    iv = Calloc(liv, int); /* a pointer */
    v = Calloc(lv, double); /* a pointer */
